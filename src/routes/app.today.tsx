...
export const Route = createFileRoute("/app/today")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Briefing harian NANTI: apa yang terlambat, jatuh tempo, dan siapa yang Anda tunggu." },
      { property: "og:title", content: "Hari ini · NANTI" },
      { property: "og:description", content: "Briefing harian NANTI: apa yang terlambat, jatuh tempo, dan siapa yang Anda tunggu." },
    ],
  }),
  component: Today,
});
  ...
  return (
    <div>
      <div className="mb-9">
        ...
        <h1 className="mt-2 text-[28px] font-semibold tracking-tight sm:text-[34px]">{greeting(settings.name)}</h1>
      </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={s.label} className="rounded-xl bg-surface px-3.5 py-3">
              <p className="text-[22px] font-semibold leading-none tracking-tight">{s.value}</p>
      ...
      <Section title="Anda menunggu" count={waiting.length}>
        ...
        <div className="px-3 pt-2">
          <Link to="/app/waiting" className="inline-flex items-center gap-1 text-[13px] font-medium text-primary">
      ...
      {!dismissed && overdue[0] && (
        <div className="rise mb-8 rounded-xl border border-primary/25 bg-accent/60 p-5">
          ...
          <p className="mt-2.5 text-[14.5px] leading-relaxed">
            ...
            sebelumnya, tetapi saya tidak menemukan tindak lanjut setelah percakapan itu.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <Button size="sm" onClick={() => { snooze(overdue[0]!.id, 1); toast("Dijadwalkan untuk follow up"); }}>
              Follow up
            </Button>
