...
// Waiting Items
export const fetchWaitingItems = createServerFn({ method: "GET" })
  .handler(async ({ context }) => {
    const { userId } = context as { userId: string };
    const supabase = getAdminClient();
    const { data, error } = await supabase
...
// Inbox Items
export const fetchInboxItems = createServerFn({ method: "GET" })
  .handler(async ({ context }) => {
    const { userId } = context as { userId: string };
    const supabase = getAdminClient();
    const { data, error } = await supabase
...
// AI Messages
export const fetchAiMessages = createServerFn({ method: "GET" })
  .handler(async ({ context }) => {
    const { userId } = context as { userId: string };
    const supabase = getAdminClient();
    const { data, error } = await supabase
...
export const seedDemoData = createServerFn({ method: "POST" })
  .handler(async ({ context }) => {
    ...
    const { data: projects } = await supabase
      .from("projects")
      .insert([
        { user_id: userId, name: "ABC Export Order", description: "Order 500 pcs untuk PT ABC, termasuk quotation, katalog dan produksi.", color: "#25D366" },
        { user_id: userId, name: "Bali Villa Project", description: "Pembangunan villa 3 unit di Canggu, koordinasi kontraktor dan owner.", color: "#34B7F1" },
        { user_id: userId, name: "Marketing Campaign", description: "Kampanye Ramadhan: artwork, konten dan jadwal posting.", color: "#FF6B6B" },
        { user_id: userId, name: "Factory Operations", description: "Operasional pabrik harian dan laporan produksi.", color: "#FFD93D" },
      ])
      .select();
    ...
    const { data: people } = await supabase
      .from("people")
      .insert([
        { user_id: userId, name: "Budi Santoso", company: "PT ABC Export", role: "Purchasing Manager" },
        { user_id: userId, name: "Siska Amelia", company: "Marketing Team", role: "Graphic Designer" },
        { user_id: userId, name: "Pak Hendra", company: "Factory Operations", role: "Kepala Produksi" },
