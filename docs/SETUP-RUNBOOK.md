...
Six steps, in order. Everything else in code is done. Tick each box as you go.
...
I checked the live database against the migration folder. **22 of 24 migrations already ran. Two never did**, and they are the ones that create `reminders`, `invoices`, `ai_clarifications`, `blog_articles`, `audit_log` and friends.
...
| `blog_articles` | Blog (currently falls back to static files — the PGRST205 you saw) |
...
That file is the two missing migrations plus the blog seed, made safe to re-run (it only differs by `CREATE INDEX IF NOT EXISTS` and a `DROP POLICY IF EXISTS` before each policy; the seed uses `ON CONFLICT DO NOTHING`). Running it twice is harmless.
...
> The other 22 migrations are already applied — do **not** run them again.
...
- **Leaked password protection**: ON (https://supabase.com/dashboard/project/qyfywekaorkwpzrvbrth/auth/settings — scroll to *Password Protection*)
- **SMTP**: https://supabase.com/dashboard/project/qyfywekaorkwpzrvbrth/auth/smtp — switch from Inbucket to a real provider (Resend / Brevo / Gmail app password) so signup + reset mails actually deliver.
...
Generate the two secrets locally — run in the project folder:
...
6. **Deployments** → top deployment → **⋯** → **Redeploy**.
...
`src/routes/legal.terms.tsx` and `src/routes/legal.privacy.tsx` are both marked `DRAFT — requires legal review before publication`.
